  ...
  values = [
    templatefile(
      "${path.module}/nocodb-values.yaml",
      {
        ...
        db_port     = 6432
        db_dbname   = yandex_mdb_postgresql_database.nocodb_db.0.name
        db_username = yandex_mdb_postgresql_user.nocodb_user.0.name
        db_password = random_string.nocodb_password.0.result
